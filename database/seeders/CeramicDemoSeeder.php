...
class CeramicDemoSeeder extends Seeder
{
    ...
    private function createCategories()
    {
        ...
        foreach ($categories as $index => $category) {
            $cat = Category::create([
                ...
                'parent_id' => 0, // 0 for main categories (parent_id cannot be null)
    ...
    private function createProducts()
    {
        ...
        foreach ($products as $index => $productData) {
            ...
            // Generate image names based on product code
            // To use real images: 
            // 1. Download images from RAK Ceramics website
            ...
            $imageName = $codeLower . '.webp'; // e.g., cft-001.webp
            $thumbnailName = $codeLower . '-thumb.webp'; // e.g., cft-001-thumb.webp
            
            // Use default placeholder image that exists in the system
            // The system will show placeholder if image doesn't exist
            // You can upload real images later via admin panel
            $defaultImage = 'def.webp'; // Default placeholder image
            $defaultThumb = 'def.webp'; // Default thumbnail
            
            // Check if product-specific image exists, otherwise use default
            $productImagePath = storage_path('app/public/product/' . $imageName);
            $productThumbPath = storage_path('app/public/product/thumbnail/' . $thumbnailName);
            
            // Use Storage facade to check if file exists (more reliable)
            $finalImageName = Storage::disk('public')->exists('product/' . $imageName) 
                ? $imageName 
                : $defaultImage;
            $finalThumbName = Storage::disk('public')->exists('product/thumbnail/' . $thumbnailName) 
                ? $thumbnailName 
                : $defaultThumb;
            ...
            $product = Product::create([
                ...
                'colors' => json_encode([]), // Empty JSON array - colors cannot be null
                'choice_options' => json_encode([]), // Empty JSON array - choice_options cannot be null
                'variation' => json_encode([]), // Empty JSON array - variation cannot be null
                ...
                'color_image' => json_encode([]), // Empty JSON array - color_image cannot be null
                'meta_title' => $productData['en_name'] . ' - Premium Ceramic Tiles',
                'meta_description' => $productData['en_details'],
                'meta_image' => $finalThumbName,
                'video_provider' => null,
                'video_url' => null,
                'attributes' => json_encode([]), // Empty JSON array - attributes cannot be null
            ]);
